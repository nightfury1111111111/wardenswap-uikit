...
const useModal = (
...
): [Handler, Handler, Dispatch<SetStateAction<string>>|undefined] => {
  const { isOpen, nodeId, modalNode, setModalNode, onPresent, onDismiss, setCloseOnOverlayClick, setBackground } = useContext(Context);
  ...
  return [onPresentCallback, onDismiss, setBackground];
};
